...
const AI_FAVOURED_TOKENS = new Set([
  ...
  'leverage','streamline','optimize','orchestrate','instantiate','encapsulate',
  'traverse','iterate','aggregate','decouple','designate','configure','invoke',
  'serialize','deserialize','authenticate','authorize','sanitize','normalize',
  'interpolate','propagate','allocate','deallocate','dispatch','intercept',
  'transform','evaluate','concatenate','enumerate','specify','construct',

  // Adjectives / adverbs
  'comprehensive','appropriate','efficient','effective','robust','seamless',
  'modular','scalable','maintainable','reusable','extensible','immutable',
  'asynchronous','concurrent','idempotent','deterministic','polymorphic',
  'declarative','imperative',

  // Nouns
  'functionality','implementation','methodology','paradigm','abstraction',
  'parameters','arguments','returns','raises','attributes','methods','instances',
  'middleware','endpoint','payload','schema','serializer','validator',
  'repository','singleton','factory','observer','decorator','adapter',
  'configuration','dependency','initializer','constructor','destructor',
  ...
  'moreover','nevertheless','specifically','essentially','fundamentally',
]);
...
const HUMAN_FAVOURED_TOKENS = new Set([
  ...
  'src','dst','ctx','req','res','cb','fn','str','len','pos','cur','prev',
  'fmt','pkg','desc','max','min','avg','acc','sum',

  // Casual / informal
  'hack','quick','temp','fixme','todo','wtf','idk','check','maybe','rough',
  'kludge','nah','hmm','ugh','wip','nope','yep','aka','misc','junk',
  'crap','oops','meh','stub','mock','dummy','workaround','bandaid','monkey',
  'patch','sketchy','janky','brittle','flaky','dodgy',

  // Short practical words
  'old','new','bad','good','test','debug','fix','try','run','get','set','add',
  'del','pop','push','swap','skip','done','fail','pass','drop','grab','tweak',
]);
...
export function bayesianFeatureScore(text, lines) {
  const features = [
    // ── Strong AI features ──
    { test: () => /\b(ensure|validate|initialize|implement|leverage|orchestrate)\b/i.test(text),  w: 8,  ai: true  },
    ...
    { test: () => /\b(comprehensive|appropriate|efficiently|seamless|robust)\b/i.test(text),      w: 6,  ai: true  },
    ...
    { test: () => (text.match(/^\s*(#|\/\/)\s*(Step|Phase)\s+\d+/gm) || []).length >= 2,          w: 9,  ai: true  },

    // Consistent trailing commas in multi-line objects/arrays (AI pattern)
    { test: () => (text.match(/,\s*\n\s*[}\]]/g) || []).length >= 3,                              w: 5,  ai: true  },

    // Verbose error messages
    { test: () => {
        const errMsgs = text.match(/(raise|throw|Error|Exception)\s*\(\s*["'`]([^"'`]+)["'`]/g) || [];
        return errMsgs.some(m => { const inner = m.match(/["'`]([^"'`]+)["'`]/); return inner && inner[1].length > 40; });
      },                                                                                           w: 6,  ai: true  },

    // ── Strong human features ──
    { test: () => /\bTODO\b|\bFIXME\b|\bHACK\b|\bWIP\b|\bXXX\b/i.test(text),                    w: 9,  ai: false },
    { test: () => /^\s*except\s*:/m.test(text),                                                    w: 7,  ai: false },
    { test: () => /\b(tmp|buf|ret|idx|cnt|ctx|cb|ptr)\b/.test(text),                              w: 6,  ai: false },
    ...
    { test: () => lines.some(l => /\d{2,}/.test(l) && !/=\s*\d/.test(l) && !/["']/.test(l)),      w: 4,  ai: false },

    // Mixed quote styles (human signal — inconsistency)
    { test: () => {
        const singles = (text.match(/'/g) || []).length;
        const doubles = (text.match(/"/g) || []).length;
        return singles > 3 && doubles > 3 && Math.min(singles, doubles) / Math.max(singles, doubles) > 0.3;
      },                                                                                           w: 5,  ai: false },

    // console.log with just variables (debug leftover, human)
    { test: () => /console\.log\s*\(\s*[a-z_]\w*\s*\)/m.test(text),                              w: 6,  ai: false },

    // Short error messages (human)
    { test: () => {
        const errMsgs = text.match(/(raise|throw|Error)\s*\(\s*["'`]([^"'`]+)["'`]/g) || [];
        return errMsgs.some(m => { const inner = m.match(/["'`]([^"'`]+)["'`]/); return inner && inner[1].length < 15; });
      },                                                                                           w: 5,  ai: false },
  ];
