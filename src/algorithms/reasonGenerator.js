...
const SIGNAL_PHRASES = {
  "docstring delimiter":
    "contains docstring delimiters — AI tools add these to every function, even trivial ones",
  "structured docstring section (Args/Returns)":
    "has a formal Args/Returns section — real developers rarely write these",
  "fully type-annotated function signature":
    "has full type annotations on every parameter — a strong AI pattern",
  "return type annotation":
    "declares a return type — AI annotates every return; humans usually skip it",
  "broad exception catch with alias (AI pattern)":
    "uses 'except Exception as e' — AI's default; humans write bare or specific catches",
  "typing module import":
    "imports from the typing module — AI always adds this for List, Dict, Optional etc.",
  "formal sentence-style comment":
    "has a comment written as a full sentence — AI writes these; humans dash off quick notes",
  "casual comment marker":
    "contains a TODO or casual note — developers leave these; AI tools never do",
  "lowercase terse comment":
    "has a short, lowercase comment — the kind written quickly while coding",
  "one-liner shortcut":
    "uses a compact one-liner — a human shortcut AI usually writes across multiple lines",
  "AI-typical demo variable":
    "uses 'test_cases' — almost exclusively seen in AI-generated tutorial code",
  "bare except (human shortcut)":
    "has a bare 'except:' — a human shortcut that AI never writes",
  "TODO/FIXME comment":
    "has a TODO/FIXME — a developer reminder to themselves; AI tools never leave these",
  "emoji in code (AI pattern)":
    "contains emoji characters — AI tools decorate output with emojis; human developers almost never do this in code",
};

const DYNAMIC_PREFIXES = [
  {
    prefix: "verbose naming:",
    tmpl: (extra) =>
      `uses long descriptive names like '${extra}' — AI tools name for clarity; humans abbreviate`,
  },
  {
    prefix: "abbreviated names:",
    tmpl: (extra) =>
      `uses short names like '${extra}' — a developer writing quickly`,
  },
];
...
function renderPhrase(signal) {
  ...
  for (const { prefix, tmpl } of DYNAMIC_PREFIXES) {
    if (signal.startsWith(prefix))
      return tmpl(signal.slice(prefix.length).trim());
  }
  return null;
}
...
export function buildGroupReason(group) {
  ...
  if (dominant === "ai") {
    if (!phrases.length)
      return `${range} follow highly consistent formatting and naming patterns typical of AI-generated code.`;
    if (phrases.length === 1) return `${range} ${phrases[0]}.`;
    return `${range} ${phrases[0]}. It also ${phrases[1]}.`;
  }
  if (dominant === "human") {
    if (!phrases.length)
      return `${range} show an informal, practical style — code written to get the job done.`;
    if (phrases.length === 1) return `${range} ${phrases[0]}.`;
    return `${range} ${phrases[0]}. Additionally, it ${phrases[1]}.`;
  }
  if (!phrases.length)
    return `${range} shows a blend of AI and human patterns — possibly edited AI-generated code.`;
...
export function buildOverallExplanation(result) {
  ...
  // Tier 2 evidence
  if (m2.log_rank > 65)
    aiEvidence.push("vocabulary skews heavily toward formal AI-favoured words");
  if (m2.bayesian_features > 65)
    aiEvidence.push("multiple Bayesian features matched known AI patterns");
  if (100 - m2.mattr > 55)
    aiEvidence.push("low moving-average vocabulary richness across segments");
  ...
  if (isAI) {
    ...
    if (!suggestions.length)
      suggestions = [
        "Try using shorter variable names and removing the formal documentation structure",
      ];
  } else if (isMixed) {
    headline =
      "This code appears to be a mix of AI-generated and human-written sections.";
    const aiParts = aiEvidence.slice(0, 2);
    const humanParts = humanEvidence.slice(0, 2);
    body =
      `The AI-like sections show ${aiParts.length ? aiParts.join(" and ") : "structured, formal patterns"}. ` +
    ...
    suggestions = [
      overall_score > 25 &&
        "A few sections scored slightly AI-like — check the yellow-highlighted lines",
